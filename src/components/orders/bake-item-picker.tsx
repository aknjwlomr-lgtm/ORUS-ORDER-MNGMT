...
import { addFreshBakeItem, removeFreshBakeItem } from "@/app/(app)/orders/actions";

export function BakeItemPicker({
  value,
  onChange,
  base,
  custom,
  ...
  // Owned by the parent form and shared across every item row.
  custom: string[];
  ...
  function addCustom() {
    ...
    setError(null);
    start(async () => {
      const res = await addFreshBakeItem(name);
      if (res.ok) {
        onCustomChange(res.items);
        onChange(name);
        setNewName("");
        setAdding(false);
        setOpen(false);
  ...
  function removeCustom(name: string) {
    start(async () => {
      const res = await removeFreshBakeItem(name);
      if (res.ok) {
        onCustomChange(res.items);
  ...
  return (
    <div ref={wrapRef} className="relative">
      ...
      >
        <span className={cn("block truncate uppercase", !value && "text-foreground/30")}>{value || "Select item"}</span>
      </button>
      {value && (
        <button
          type="button"
