...
import type { AttributeResult } from "@/app/(app)/orders/actions";

/**
 * Dropdown that lets staff pick from a built-in list plus their own saved
 * additions, and create/delete custom values inline. The add/remove server
 * actions are passed in so the same component works for flavor, shape, weight, …
 */
export function AttributePicker({
  ...
  // The custom list is owned by the parent form and shared across every picker
  // instance, so a value created in one row appears in all rows (and new ones).
  custom: string[];
  onCustomChange: (list: string[]) => void;
  onAdd: (name: string) => Promise<AttributeResult>;
  onRemove: (name: string) => Promise<AttributeResult>;
  selectLabel: string;
  createLabel: string;
  ...
  return (
    <div ref={wrapRef} className="relative">
      ...
      >
        <span className={cn("block truncate uppercase", !value && "text-foreground/30")}>{value || selectLabel}</span>
      </button>
      {value && (
        <button
          type="button"
